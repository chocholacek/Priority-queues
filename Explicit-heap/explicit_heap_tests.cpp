...
using min_heap = PriorityQueues::explicit_heap< int, ary, std::less< int > >;
...
template < std::size_t ary >
min_heap< ary > generate_min_heap(std::size_t vector_size, vector_t type) {
    min_heap< ary > h;
...
TEST_CASE("Normal - insert") {
    ...
    SECTION("binary") {
        std::size_t size = 10;
        auto h = generate_min_heap< 2 >(size, type);
    ...
    SECTION("4-ary"){
        auto h = generate_min_heap< 4 >(10, vector_t::normal);
...
TEST_CASE("Reversed - insert") {
    ...
    SECTION("binary") {
        auto h = generate_min_heap< 2 >(10, type);
